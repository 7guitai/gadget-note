...
## ローカル実行・Remote Controlとの違い

| | クラウドセッション | Remote Control | ローカル実行 |
| --- | --- | --- | --- |
| 処理が動く場所 | Anthropicが管理する仮想マシン | 自分のPC | 自分のPC |
| PCを閉じたとき | 作業を続けられる | PCで動かし続ける必要がある | PCで動かし続ける必要がある |
| 手元のPCだけにあるファイル | 使えない（GitHubに用意する） | 使える | 使える |
| スマホからの確認・指示 | できる | できる | できない |
| 始め方 | claude.ai/code、Claudeアプリ、`claude --cloud` など | PCのClaude Codeから接続 | ターミナルやデスクトップアプリ |
