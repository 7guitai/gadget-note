...
function check(slug: string, meta: ArticleMeta, raw: string) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(meta.date))) problems.push('date は引用符付きの "YYYY-MM-DD" にしてください');
  if (meta.cover && !existsSync(`public${meta.cover}`)) problems.push(`cover の画像が見つかりません: public${meta.cover}`);
  if (meta.cover && !meta.coverAlt) problems.push('cover を使うときは coverAlt（画像の説明）も必要です');
  ...
  if (!meta.draft) {
    ...
    for (const tag of raw.match(/<a\s[^>]*>/g) ?? []) {
      ...
      if (!meta.affiliate) problems.push('アフィリエイトリンクがあるので affiliate: true にしてください');
  ...
  if (problems.length) throw new Error(`記事 ${slug} を確認してください:\n- ${[...new Set(problems)].join('\n- ')}`);
}
