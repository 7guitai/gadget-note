...
// 本文の画像・仮置き・購入リンクの共通チェック（記事・ニュースで共用）
export function checkBody(raw: string, opts: { draft?: boolean; affiliate?: boolean; cover?: string; coverAlt?: string }) {
...
export const isDate = (d: unknown) => /^\d{4}-\d{2}-\d{2}$/.test(String(d));
export const todayJst = () => new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Tokyo' });
export function fail(label: string, problems: string[]) {
