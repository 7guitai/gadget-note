...
export const GET: APIRoute = ({ site }) => {
  const paths = ['/', '/about/', '/privacy/', ...categories.filter(c => articles.some(a => a.meta.category === c.id)).map(c => `/category/${c.id}/`), ...articles.map(a => `/articles/${a.slug}/`), '/news/', ...news.map(n => `/news/${n.slug}/`)];
